...
def test_processor_context_returns_processor_context(model, license_key):
    processor = create_processor_or_skip(model, license_key)
    ctx = processor.get_context()
    assert isinstance(ctx, aic.ProcessorContext)


def test_processor_context_get_audio_delay_returns_int(model, license_key):
    processor = create_processor_or_skip(model, license_key)
    config = aic.ProcessorConfig(48000, 480, False)
    processor.initialize(config)
    ctx = processor.get_context()
    delay = ctx.get_audio_delay()
    assert isinstance(delay, int)


def test_processor_context_get_audio_delay_is_non_negative(model, license_key):
    processor = create_processor_or_skip(model, license_key)
    config = aic.ProcessorConfig(48000, 480, False)
    processor.initialize(config)
    ctx = processor.get_context()
