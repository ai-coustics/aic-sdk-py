...
def test_process_deterministic_output(model, license_key, test_audio_path):
    """Test that processing the same file produces identical output on multiple runs."""
    processor = create_processor_or_skip(model, license_key)
    ...
    outputs = []
    for _ in range(2):
        proc = create_processor_or_skip(model, license_key)
        proc.initialize(config)
        proc_ctx = proc.get_processor_context()
        proc_ctx.set_parameter(aic.ProcessorParameter.EnhancementLevel, 0.9)
        proc_ctx.set_parameter(aic.ProcessorParameter.VoiceGain, 0.9)
        output = proc.process(audio.copy())
        outputs.append(output)

    assert np.allclose(outputs[0], outputs[1], rtol=1e-10, atol=1e-10)
