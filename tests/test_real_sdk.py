...
def test_real_sdk_model_processing(model, license_key):
    ...
    # initial processor
    config_initial = aic.ProcessorConfig(probe_sr, num_frames, allow_variable_frames)
    ...
    config = aic.ProcessorConfig(optimal_sr, optimal_frames, allow_variable_frames)
    processor = aic.Processor(model, license_key)
    processor.initialize(config)
    proc_ctx = processor.get_processor_context()
    proc_ctx.set_parameter(aic.ProcessorParameter.EnhancementLevel, 0.8)
    audio = make_sine_noise(optimal_frames * 10, sr=optimal_sr)
    original = audio.copy()

    for s, e in chunks(audio.shape[0], optimal_frames):
        chunk = audio[s:e]
        if chunk.shape[0] < optimal_frames:
            padded = np.zeros(optimal_frames, dtype=audio.dtype)
            padded[: chunk.shape[0]] = chunk
            processed = processor.process(padded)
            audio[s:e] = processed[: chunk.shape[0]]
        else:
...
def test_real_sdk_auto_model_selection(model, license_key):
    ...
    num_frames = 240
    processor = aic.Processor(model, license_key)
    config = aic.ProcessorConfig(sample_rate, num_frames, False)
    processor.initialize(config)
    optimal_sr = model.get_optimal_sample_rate()
    optimal_frames = model.get_optimal_num_frames(optimal_sr)
