...
async def test_download_returns_path_string(tmp_path, use_async):
    model_path = await _download_model("rook-s-48khz", str(tmp_path), use_async)
...
async def test_downloaded_model_has_correct_id(tmp_path, use_async):
    model_path = await _download_model("rook-s-48khz", str(tmp_path), use_async)
    model = aic.Model.from_file(model_path)
    assert "rook" in model.get_id().lower()
...
async def test_download_to_nested_directory(tmp_path, use_async):
    ...
    model_path = await _download_model("rook-s-48khz", str(nested_dir), use_async)
    assert os.path.exists(model_path)
...
async def test_download_same_model_twice_succeeds(tmp_path, use_async):
    model_path1 = await _download_model("rook-s-48khz", str(tmp_path), use_async)
    model_path2 = await _download_model("rook-s-48khz", str(tmp_path), use_async)
...
async def test_download_accepts_path_object(tmp_path, use_async):
    ...
    model_path = await _download_model("rook-s-48khz", download_dir, use_async)
    assert isinstance(model_path, str)
    assert os.path.exists(model_path)
    assert model_path.endswith(".aicmodel")
...
def test_from_file_accepts_path_object(tmp_path):
    ...
    download_dir = Path(tmp_path)
    model_path_str = aic.Model.download("rook-s-48khz", download_dir)
